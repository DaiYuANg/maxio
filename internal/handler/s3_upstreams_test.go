...
func TestS3UpstreamRoutesCreateListGetAndDelete(t *testing.T) {
	...
	create := serveRouterRequest(router, http.MethodPost, "/_s3/upstreams", nil, body)
	if create.Code != http.StatusOK {
		t.Fatalf("create status = %d body = %s", create.Code, create.Body.String())
	}

	list := serveRouterRequest(router, http.MethodGet, "/_s3/upstreams", nil, nil)
	...
	get := serveRouterRequest(router, http.MethodGet, "/_s3/upstreams/u-1", nil, nil)
	if get.Code != http.StatusOK {
		t.Fatalf("get status = %d body = %s", get.Code, get.Body.String())
	}

	del := serveRouterRequest(router, http.MethodDelete, "/_s3/upstreams/u-1", nil, nil)
...
func TestS3UpstreamRoutesRequireAdminAuth(t *testing.T) {
	...
	recorder := serveRouterRequest(router, http.MethodGet, "/_s3/upstreams", nil, nil)
...
func newS3UpstreamRouter(t *testing.T, cfg config.Config) http.Handler {
	...
	deps := handler.NewDependencies(nil, nil, nil, nil, metadata.NewInMemoryMetadata())
