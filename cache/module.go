...
func Module() dix.Module {
	return dix.NewModule(
		...
		dix.WithModuleProviders(
			...
			dix.Provider3(func(storage *store.Store, metadata MetadataCache, logger *slog.Logger) object.Store {
				return NewStore(storage, metadata, logger)
			}),
		),
		dix.Hooks(
...
func newMetadataCacheFromRuntimeConfig(cfg config.Config, logger *slog.Logger) (MetadataCache, error) {
	cacheConfig := Config{
	...
	case "memory":
		return NewMemoryMetadataCache(MemoryConfig{
			TTL:     cacheConfig.TTL,
